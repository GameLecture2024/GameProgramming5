...
*  탐욕 알고리즘 (Greedy)
*  문제 풀이 방식의 하나이다.
*  최적의 해를 찾기 위해서 현재 상황에서 가장 최선의 방법을 계속해서
   선택하는 방식입니다.
*/
...
*  n원을 동전으로만 거슬러 줘야하는데 동전의 종류
   500원, 100원, 50원, 10원가 있다. 이 때 동전을 최소한의 갯수로 거슬러주고 싶을 때 
...
// 500 x 2 + 100 x 2 + 50 x 1 + 10 x 1 = 2+2+1+1 = 6
// 백준 14916 문제
...
//나누기 나머지를 사용하기
	...
	*/
	...
	std::vector<int> coins = {500, 100, 50, 10, 5,1};	// 여기의 데이터만 변경하세요.
...
int meetingCount(int n)
{
	...
	// 끝나는 시간 순으로 정렬한다. 끝나는 시간을 key값을 first에 저장하였다.
	...
	// 현재 선택된 회의의 끝나는 시간과 나중에 선택된 회의의 시작시작을 비교한다.
