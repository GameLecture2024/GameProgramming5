...
bool circleCheck(Circle c1, Circle c2)
{
	...
	double dx = c1.x - c2.x; // 가로의 길이. 제곱을 해준다. 타입이 음수가 사용될 수 있어야한다.
	double dy = c1.y - c2.y;
...
public:
	...
	{
		if (collisionCheck(collider, other.collider))  // 충돌했을 떄
		{
			isCollision = true;
		}
		else
