...
public:
	...
	private:
		...
		void DrawLine(vector<vector<char>>& screen, int x1, int y1, int x2, int y2)
		{
			...
			while (true)
			{
				if (x >= 0 && x < CONSOLEWIDTH && y >= 0 && y < CONSOLEHEIGHT)
				{
					...
					if (x == x2 && y == y2)	 // x,y좌표가 목표지점에 도달했으면 반복을 그만둔다.
					{
						break;
					}
		...
		}    // 정수 사이의 픽셀 선을 그리는 알고리즘 Bresenham
...
int main()
{
	...
	// 0 ~ 1층
	spire.addEdge(0, 1); spire.addEdge(1, 6); spire.addEdge(6, 11); spire.addEdge(11, 15); spire.addEdge(15, 16); // 가장 왼쪽
