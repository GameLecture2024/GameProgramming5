...
//  (-> 데이터의 수가 백만개 이상(처리해야할 데이터가 많으면) 일 때 어떻게 해야 하나?)
...
// MaxHeap구현을 보고 MinHeap을 따로 구현해보기 (!!자습할 때 사용해보세요!!)
...
private:
	...
	*/
	...
	{
		if (!root) return nullptr; // 데이터가 1개도 없는 경우 실행하지마세요.
...
public:
	...
	void insert(int value)
	{
		...
		size++;
		heapifyup(newNode);
		// 현재 들어가 노드와, 그 부모의 노드를 비교해서 큰 수를 위쪽으로 보내라.
		// heapifyup
	...
	int extractMax() // 최대값을 제거하라. front,pop
	{
		...
		if (size == 1)     // 1개만 있을 때는 1개를 없애고 최대값을 반환한ㄷ.
		{
			delete root;
			root = nullptr;
			size = 0;
		...
		root->value = lastNode->value; // 제일 밑에있는 노드를 가리켰는데, 현재 root에는 마지막 값이 들어있다.

		// 지우기 위해서 가리키고 있는 가장 마지막 노드 
		if (lastNode->parentNode)
		{
			// lastNode의 값은 root로 올라갔으니까 현재 lastNode에 있는 주소를 nullptr로 바꾸기
			if (lastNode->parentNode->leftNode == lastNode) // 마지막 노드가 부모노드의 왼쪽에 있는경우
			{
