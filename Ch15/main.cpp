...
/*
*  int -> "people", "client" ( 기다리는 사람 순서, 사용 해야할 시간, 점수)
*/
...
#pragma region Q2
// 배낭에 물건 담기 문제. 백준 12865번. 물건이 N개 담을 수 있는 무게가 K가 주어집니다. 
...
void Solve2()
{
	...
	// 무게가 0 일떄? 각 무게에서 최대로 담을 수 있도록 무게별 최고 가치를 갱신하세요.
	//dp[0] = 0;
	//dp[1] = 0;
	//dp[2] = 0;
	//dp[3] = 6;
...
// 인벤토리 - 무게 시스템. - User 편의성을 위한 코드. 아이템 중에서 가장 가치잇는 아이템을 강조하는 코드를 만들어보세요.

// 이름을 클래스 만드세요 -> 아이템 클래스. 무게라는 가치가 포함되도록 만들어보세요.
...
public:
	...
	*/
	...
	{
		 // 현재 아이템의 무게에 새로운 아이템 조합을 가져올 때 저장할 컨테이너 선언
		...
		for (int i = 0; i < selectableItems.size(); i++)
		{
			...
			{
				/*
				*  해당 코드를 위의 알고리즘 코드로 작성해보세요.
				*/				
				if (dp[w - weight] + value > dp[w])  // 새로들어온 아이템 + 기존 무게  전체 무게 보다 클 때만
				{
...
{
	// 인벤토리 생성하고 호출.  4개의 아이템 이 배낭에 저장해보세요.   최대 무게가7이고, 아이템을 4개 생성.
